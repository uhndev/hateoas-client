describe('Controller: ReferralController Tests', function() {

  var scope, refCtrl, httpBackend, location;
  var API, HeaderService, AltumAPI, toastr;

  beforeEach(module('altum.referral'));
  ...
  beforeEach(inject(function (_$controller_, _$httpBackend_, _$rootScope_, $injector, $location) {
    ...
    spyOn($location, 'path').and.returnValue('/referral/1');
    refCtrl = _$controller_('ReferralController', {$scope: scope});
  ...
  describe('Basic unit tests for adding/removing service selections', function() {
    it('should at least be defined', function() {
      expect(refCtrl).toBeDefined();
    });

    it('should retrieve the correct service categories', function () {
      expect(refCtrl.currentCategories.length).toEqual(3);
    });

    it('should return unique all available altum services for a given program', function() {
      expect(refCtrl.availableServices.length).toEqual(7);
      refCtrl.availableServices.map(function (availableService) {
    ...
    it('should add available services to the recommendedServices array', function() {
      refCtrl.toggleService(refCtrl.availableServices[0]);
      refCtrl.toggleService(refCtrl.availableServices[1]);
      expect(refCtrl.recommendedServices.length).toEqual(2);
    });

    it('should un-recommend a service and make it available to be recommended again', function() {
      refCtrl.toggleService(refCtrl.recommendedServices[0]);
      expect(refCtrl.recommendedServices.length).toEqual(1);
    });
  });
  ...
  describe('Detail selection for recommended services', function() {
    ...
    it('should not be available if not all fields have been selected', function() {
      ...
      refCtrl.availableServices.map(function (availableService) {
        refCtrl.toggleService(availableService);
      });

      expect(refCtrl.areServicesValid()).toBeFalsy();
      expect(refCtrl.recommendedServices.length).toEqual(7);
    });

    it('should apply to all recommended services when selections are made in detail panel', function() {
      expect(refCtrl.validityFields.length).toEqual(7);
      refCtrl.validityFields.forEach(function (field) {
        refCtrl.setServiceSelections(field);
      });

      refCtrl.recommendedServices.forEach(function (recommendedService) {
        refCtrl.validityFields.forEach(function (field) {
    ...
    it('should be available when all fields selected and services with sites selected', function() {
      ...
      expect(refCtrl.areServicesValid()).toBeTruthy();
    });
